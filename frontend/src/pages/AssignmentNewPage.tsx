...
export default function AssignmentNewPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await assignmentApi.create({ title, description, startDate, dueDate });
      navigate('/dashboard');
    } catch (err: any) {
      setError(err.response?.data?.message || '과제 생성에 실패했습니다.');
    }
  };

  return (
    <>
      <div className="section-header"><h2>과제 생성</h2></div>
